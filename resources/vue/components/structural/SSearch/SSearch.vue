<template>
    <div id="wrapperSearch">
        <div id="containProduct">
            <label id="labelSearch" for="searchText">Rechercher</label>
            <input id="searchText" type="text" v-model="searchText">
            <UButton content="Rechercher" @onClick="handleClick"/>
            <SCardList
                :custom-style="customGridStyle"
                :products="filteredProducts"
                :withPaginate="true"
                @changePage="handleChangePage"
            />
            <span id="close" @click="handleCloseClick">
                <svg viewBox="0 0 365.71733 365" xmlns="http://www.w3.org/2000/svg"><g fill="#f44336"><path
                    d="m356.339844 296.347656-286.613282-286.613281c-12.5-12.5-32.765624-12.5-45.246093 0l-15.105469 15.082031c-12.5 12.503906-12.5 32.769532 0 45.25l286.613281 286.613282c12.503907 12.5 32.769531 12.5 45.25 0l15.082031-15.082032c12.523438-12.480468 12.523438-32.75.019532-45.25zm0 0"/><path
                    d="m295.988281 9.734375-286.613281 286.613281c-12.5 12.5-12.5 32.769532 0 45.25l15.082031 15.082032c12.503907 12.5 32.769531 12.5 45.25 0l286.632813-286.59375c12.503906-12.5 12.503906-32.765626 0-45.246094l-15.082032-15.082032c-12.5-12.523437-32.765624-12.523437-45.269531-.023437zm0 0"/></g></svg>
            </span>
        </div>
    </div>
</template>

<script>
    import {UProductCard, UButton} from '@/components/unit'
    import {SCardList} from '@/components/structural'
    import {mapGetters} from "vuex"

    export default {
        data() {
            return {
                searchText: '',
                customGridStyle: {
                    gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))',
                    overflowY: 'scroll',
                    width: '100%',
                    maxHeight: '60vh',
                    margin: '10px 0',
                    padding: 0,
                    gridGap: '10px'
                }
            }
        },
        name: "SSearch",
        methods: {
            handleClick: function () {
                this.$store.dispatch('products/getProductSearch', {
                    text: this.searchText,
                })
            },
            handleCloseClick() {
                this.$store.dispatch('search/setIsSearching', {
                    value: false,
                })
            },
            handleChangePage(page) {
                this.$store.dispatch('products/getProductSearch', {
                    text: this.searchText,
                    page
                })
            },
        },
        beforeDestroy() {
            this.$store.dispatch('products/getProductSearch', {
                text: '',
            })
        },
        computed: {
            ...mapGetters({
                filteredProducts: 'products/filteredProducts',
                isSearching: 'search/isSearching'
            }),
        },
        components: {
            UProductCard,
            UButton,
            SCardList
        }
    }
</script>
<style>
    .productCard {
        padding-top: 105% !important;
    }
</style>
<style lang="scss" scoped>
    @import "SSearch";
</style>
